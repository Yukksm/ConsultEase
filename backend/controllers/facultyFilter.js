...
const facultyFilter = async (req, res) => {
    try {
        const { facultyName } = req.body;  // Get faculty name from request

        if (!facultyName || facultyName.trim() === "") {
            return res.status(400).json({ message: "Please provide a valid faculty name" });
        }
        ...
        // Filter projects where the faculty name (column index 1) matches the entered name
        const filteredProjects = allProjects.filter(row => row[1].toLowerCase() === facultyName.toLowerCase());

        if (filteredProjects.length === 0) {
            return res.status(404).json({ message: "No projects found for the given faculty name" });
        }
